...
const destinations = ["Delhi", "Mumbai", "Chennai", "Dubai", "Goa", "Kolkata", "Bengaluru", "San Francisco"];
...
function makeBriefingData(date: string, refreshKey: number): BriefingData {
  ...
  const flights: Flight[] = Array.from({ length: 6 }, (_, index) => {
    ...
    // Ensure first flight is always San Francisco
    const destination = index === 0 ? "San Francisco" : pick(destinations, random);
